...
import traci
from plexe import Plexe, ACC, CACC, RPM, GEAR, RADAR_DISTANCE, PLOEG, DRIVER, RADAR_REL_SPEED
...
def main(demo_mode, real_engine, setter=None):
    ...
    topology = dict()
    min_dist = 1e6
    dist_log = open("plex_log.txt", "w")
 
    while running(demo_mode, step, 3000):

        # when reaching 60 seconds, reset the simulation when in demo_mode
        if demo_mode and step == 3000:
        ...
        if step == 0:
            ...
            topology = add_vehicles(plexe, 5, 1, real_engine)
...
#        if step == 1100:
#            plexe.set_fixed_acceleration(BRAKING_VEHICLE, False, 6)
        if step > 1:
            ...
            for i in range(5):
                myvid = "v.0.%d" % i
                myradar = plexe.get_radar_data(myvid)
                myvd = plexe.get_vehicle_data(myvid)
                dist_log.write("{0} {1} {2} {3} {4} {5} \n".format(step, myvid, myvd.speed, myvd.acceleration, myradar[RADAR_DISTANCE], myradar[RADAR_REL_SPEED]))
        step += 1
